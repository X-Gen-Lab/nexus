/**
 * \file            nx_isr_stm32f4.c
 * \brief           STM32F4 ISR manager implementation
 * \author          Nexus Team
 */
...
#define NX_ISR_MAX_IRQS 128

/* Maximum callbacks per IRQ */
#define NX_ISR_MAX_CALLBACKS_PER_IRQ 8
...
static nx_status_t isr_set_hw_priority(nx_isr_manager_t* self, uint32_t irq,
                                       uint8_t hw_prio);
    ...
    .base =
        {
            ...
            .set_hw_priority = isr_set_hw_priority,
            .enable = isr_enable,
            .disable = isr_disable,
        },
    .chains = {0},
...
static nx_status_t isr_set_hw_priority(nx_isr_manager_t* self, uint32_t irq,
    ...
    /* TODO: Set hardware NVIC priority */
    /* This would call NVIC_SetPriority(irq, hw_prio) */
...
static nx_status_t isr_enable(nx_isr_manager_t* self, uint32_t irq) {
    ...
    /* This would call NVIC_EnableIRQ(irq) */
...
static nx_status_t isr_disable(nx_isr_manager_t* self, uint32_t irq) {
    ...
    /* This would call NVIC_DisableIRQ(irq) */

    return NX_OK;
}

/**
 * \brief           Dispatch ISR callbacks for an IRQ
 * \note            This function should be called from the actual ISR
 */
void nx_isr_dispatch(uint32_t irq) {
    if (irq >= NX_ISR_MAX_IRQS) {
        return;
    }
