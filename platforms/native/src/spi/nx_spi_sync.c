/**
 * \file            nx_spi_sync.c
 * \brief           SPI sync interface implementation for Native platform
 * \author          Nexus Team
 * \version         1.0.0
 * \date            2026-01-18
 *
 ...
 *
 * \details         Implements SPI synchronous operations including blocking
 *                  transfer with timeout support.
 */
...
static nx_status_t tx_sync_send(nx_tx_sync_t* self, const uint8_t* data,
    ...
    nx_spi_impl_t* impl = NX_CONTAINER_OF(self, nx_spi_impl_t, tx_sync);
...
static nx_status_t tx_rx_sync_transfer(nx_tx_rx_sync_t* self,
                                       ...
                                       uint32_t timeout_ms) {
    nx_spi_impl_t* impl = NX_CONTAINER_OF(self, nx_spi_impl_t, tx_rx_sync);
    ...
    if (available >= actual_rx) {
        size_t read_count =
            spi_buffer_read(&impl->state->rx_buf, rx_data, actual_rx);
