/**
 * \file            nx_adc_native.c
 * \brief           Native platform ADC driver simulation
 * \author          Nexus Team
 */
...
/* ADC buffer size for DMA simulation */
...
typedef struct {
    ...
    uint16_t
        simulated_values[NX_ADC_MAX_CHANNELS]; /**< Simulated channel values */
} nx_adc_state_t;
...
/* ADC operations implementation (simulated) */
static nx_status_t adc_read(nx_adc_t* self, uint8_t channel, uint16_t* value) {
    ...
    /* Simulate ADC reading with random value */
    *value = impl->adc_state->simulated_values[channel];
    impl->adc_state->conversion_count++;
#ifdef NX_ADC_DEBUG
    printf("[ADC Native] Read channel %d = %d\n", channel, *value);
#endif
    return NX_OK;
}
...
static nx_status_t adc_read_multi(nx_adc_t* self, uint8_t* channels,
    ...
    for (uint8_t i = 0; i < count; i++) {
        ...
        values[i] = impl->adc_state->simulated_values[channels[i]];
    }
...
#ifdef NX_ADC_DEBUG
    printf("[ADC Native] Continuous mode started\n");
#endif
    return NX_OK;
}
...
#ifdef NX_ADC_DEBUG
    printf("[ADC Native] Continuous mode stopped\n");
#endif
    return NX_OK;
}
...
#ifdef NX_ADC_DEBUG
    printf("[ADC Native] Calibration performed\n");
#endif
    return NX_OK;
}
...
static nx_status_t adc_lifecycle_init(nx_lifecycle_t* self) {
    ...
    impl->adc_state->clock_enabled = true;
    impl->adc_state->initialized = true;
    /* Initialize simulated values with random data */
    for (int i = 0; i < NX_ADC_MAX_CHANNELS; i++) {
        impl->adc_state->simulated_values[i] = (uint16_t)(rand() % 4096);
    }
#ifdef NX_ADC_DEBUG
    printf("[ADC Native] ADC %d initialized\n", impl->adc_state->adc_index);
#endif
    return NX_OK;
}
...
static nx_status_t adc_lifecycle_deinit(nx_lifecycle_t* self) {
    ...
    impl->adc_state->busy = false;
    impl->adc_state->clock_enabled = false;
    impl->adc_state->initialized = false;
#ifdef NX_ADC_DEBUG
    printf("[ADC Native] ADC %d deinitialized\n", impl->adc_state->adc_index);
#endif
    return NX_OK;
}
...
static bool adc_power_is_enabled(nx_power_t* self) {
    ...
    return impl->adc_state->clock_enabled;
}
...
nx_adc_t* nx_adc_native_get_with_config(uint8_t adc_index,
    ...
    nx_adc_t* adc = nx_adc_native_get(adc_index);
...
void nx_adc_native_set_simulated_value(uint8_t adc_index, uint8_t channel,
                                       uint16_t value) {
    if (adc_index >= NX_ADC_MAX_COUNT || channel >= NX_ADC_MAX_CHANNELS) {
        return;
    }
    g_adc_states[adc_index].simulated_values[channel] = value;
}
 ...
 */
nx_device_t* nx_adc_native_get_device(uint8_t index) {
