...
/**
 * \brief           Reset all CRC instances (for testing)
 */
void nx_crc_native_reset_all(void) {
    for (uint8_t i = 0; i < g_crc_instance_count; i++) {
        nx_crc_impl_t* impl = &g_crc_instances[i];
        if (impl->state && impl->state->initialized) {
            impl->lifecycle.deinit(&impl->lifecycle);
        }
        memset(&g_crc_states[i], 0, sizeof(nx_crc_state_t));
    }
    g_crc_instance_count = 0;
}

/**
 * \brief           Reset CRC instance (for testing)
 */
nx_status_t nx_crc_native_reset(uint8_t index) {
    if (index >= g_crc_instance_count) {
        return NX_ERR_INVALID_PARAM;
    }

    nx_crc_impl_t* impl = &g_crc_instances[index];
    if (impl->state) {
        crc_reset_state(impl->state);
        return NX_OK;
    }
...
/**
 * \brief           Get CRC state (for testing)
 */
...
/**
 * \brief           Get CRC device descriptor (for testing)
 */
nx_device_t* nx_crc_native_get_device(uint8_t index) {
    ...
    return g_crc_instances[index].device;
}
