...
/**
 * \brief           Get number of registered callbacks for an IRQ (test
 * interface)
...
uint8_t nx_isr_test_get_callback_count(uint32_t irq) {
    ...
    return g_isr_manager.chains[irq].count;
}

/**
 * \brief           Check if an IRQ is enabled (test interface)
 * \param[in]       irq: IRQ number
...
bool nx_isr_test_is_enabled(uint32_t irq) {
    ...
    return g_isr_manager.chains[irq].enabled;
}

/**
 * \brief           Reset all ISR registrations (test interface)
 * \details         Clears all callback chains and resets state
 */
void nx_isr_test_reset_all(void) {
    for (size_t i = 0; i < NX_ISR_MAX_IRQS; i++) {
        g_isr_manager.chains[i].head = NULL;
        g_isr_manager.chains[i].count = 0;
        g_isr_manager.chains[i].enabled = false;
    }

    for (size_t i = 0; i < sizeof(g_isr_manager.handle_pool) /
                               sizeof(g_isr_manager.handle_pool[0]);
         i++) {
        g_isr_manager.handle_pool[i].active = false;
    }
}

/**
 * \brief           Get total number of active handles (test interface)
