...
/**
 * \brief           Get number of allocated DMA channels (test interface)
...
uint8_t nx_dma_test_get_allocated_count(uint8_t dma_index) {
    ...
    uint8_t count = 0;
    for (uint8_t i = 0; i < NX_DMA_MAX_CHANNELS_PER_CTRL; i++) {
        if (g_dma_channels[dma_index][i].state != NX_DMA_CH_STATE_FREE) {
            count++;
        }
    }
...
/**
 * \brief           Check if a specific channel is allocated (test interface)
 * \param[in]       dma_index: DMA controller index
...
bool nx_dma_test_is_channel_allocated(uint8_t dma_index, uint8_t channel) {
    ...
    return g_dma_channels[dma_index][channel].state != NX_DMA_CH_STATE_FREE;
}

/**
 * \brief           Reset all DMA channels (test interface)
 * \details         Releases all allocated channels and resets state
 */
void nx_dma_test_reset_all(void) {
    for (uint8_t i = 0; i < NX_DMA_MAX_CONTROLLERS; i++) {
        for (uint8_t j = 0; j < NX_DMA_MAX_CHANNELS_PER_CTRL; j++) {
            g_dma_channels[i][j].state = NX_DMA_CH_STATE_FREE;
            g_dma_channels[i][j].callback = NULL;
            g_dma_channels[i][j].user_data = NULL;
            g_dma_channels[i][j].remaining = 0;
            memset(&g_dma_channels[i][j].config, 0, sizeof(nx_dma_config_t));
        }
    }
}

/**
 * \brief           Simulate DMA transfer completion (test interface)
