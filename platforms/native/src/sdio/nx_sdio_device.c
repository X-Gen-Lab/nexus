...
/**
 * \brief           Get SDIO instance
 */
nx_sdio_t* nx_sdio_native_get(uint8_t index) {
    if (index >= g_sdio_instance_count) {
        return NULL;
    }
    return &g_sdio_instances[index].base;
}
...
void nx_sdio_native_reset_all(void) {
    for (uint8_t i = 0; i < g_sdio_instance_count; i++) {
        sdio_reset_state(&g_sdio_states[i]);
    }
}
...
nx_status_t nx_sdio_native_reset(uint8_t index) {
    ...
    sdio_reset_state(&g_sdio_states[index]);
    return NX_OK;
}
...
nx_status_t nx_sdio_native_get_state(uint8_t index, bool* initialized,
    ...
    nx_sdio_state_t* state = &g_sdio_states[index];
    if (initialized) {
...
struct nx_device_s* nx_sdio_native_get_device(uint8_t index) {
    ...
    return g_sdio_instances[index].device;
}
...
nx_status_t nx_sdio_native_set_card_present(uint8_t index, bool present) {
    ...
    g_sdio_states[index].card_present = present;
    return NX_OK;
}
...
bool nx_sdio_native_is_card_present(uint8_t index) {
    ...
    return g_sdio_states[index].card_present;
}
...
nx_status_t nx_sdio_native_get_block_data(uint8_t index, uint32_t block,
    ...
    nx_sdio_state_t* state = &g_sdio_states[index];
    memcpy(data, state->blocks[block].data, NX_SDIO_BLOCK_SIZE);
    return NX_OK;
}
