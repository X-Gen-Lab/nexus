...
TEST_F(HistoryPropertyTest, Property5_HistoryFIFOOrder) {
    for (int iter = 0; iter < PROPERTY_TEST_ITERATIONS; ++iter) {
        ...
        /* History should contain the most recent min(cmdCount, capacity) commands */
        int expectedCount = std::min(cmdCount, static_cast<int>(HISTORY_CAPACITY));
        ...
        for (int i = 0; i < expectedCount; ++i) {
            ...
            const char* entry = history_get_entry(&history, static_cast<uint8_t>(i));
        ...
        for (int i = 0; i < expectedCount; ++i) {
            ...
            EXPECT_STREQ(commands[srcIdx].c_str(), prev)
                << "Iter " << iter << ": navigation order violated at step " << i;
        }
    }
}
...
TEST_F(HistoryPropertyTest, Property5b_CapacityOverflowRemovesOldest) {
    for (int iter = 0; iter < PROPERTY_TEST_ITERATIONS; ++iter) {
        ...
        std::vector<std::string> commands = generateUniqueCommands(totalCommands);
...
TEST_F(HistoryPropertyTest, Property6_HistoryDeduplication) {
    for (int iter = 0; iter < PROPERTY_TEST_ITERATIONS; ++iter) {
        ...
        std::vector<std::string> baseCommands = generateUniqueCommands(baseCount);
        
        /* Build sequence with random consecutive duplicates */
        std::vector<std::string> sequence;
        std::vector<std::string> expectedHistory;  /* What should end up in history */
...
/**
 * Feature: shell-cli-middleware, Property 6a: Non-Consecutive Duplicates Allowed
 * 
...
TEST_F(HistoryPropertyTest, Property6a_NonConsecutiveDuplicatesAllowed) {
    for (int iter = 0; iter < PROPERTY_TEST_ITERATIONS; ++iter) {
        ...
        EXPECT_EQ(expectedCount, history_get_count(&history))
            << "Iter " << iter << ": non-consecutive duplicates should be stored";
    }
}
    ...
    for (int iter = 0; iter < PROPERTY_TEST_ITERATIONS; ++iter) {
        ...
        for (int i = 0; i < len; ++i) {
            switch (wsDist(rng)) {
                ...
                case 1: whitespace += '\t'; break;
                case 2: whitespace += '\n'; break;
                case 3: whitespace += '\r'; break;
            }
        }
