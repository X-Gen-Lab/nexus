...
TEST_F(CommandTest, RegisterBeyondCapacityRejected) {
    ...
    /* Try to add one more */
    snprintf(names[SHELL_MAX_COMMANDS], sizeof(names[SHELL_MAX_COMMANDS]), "overflow");
...
TEST_F(CommandTest, UnregisterMiddleCommand) {
    shell_command_t cmd1 = { .name = "cmd1", .handler = test_handler, .help = nullptr, .usage = nullptr, .completion = nullptr };
    shell_command_t cmd2 = { .name = "cmd2", .handler = test_handler, .help = nullptr, .usage = nullptr, .completion = nullptr };
    shell_command_t cmd3 = { .name = "cmd3", .handler = test_handler, .help = nullptr, .usage = nullptr, .completion = nullptr };
...
TEST_F(CommandTest, GetCommandFromMultiple) {
    shell_command_t cmd1 = { .name = "alpha", .handler = test_handler, .help = nullptr, .usage = nullptr, .completion = nullptr };
    shell_command_t cmd2 = { .name = "beta", .handler = test_handler2, .help = nullptr, .usage = nullptr, .completion = nullptr };
    shell_command_t cmd3 = { .name = "gamma", .handler = test_handler, .help = nullptr, .usage = nullptr, .completion = nullptr };
