  ...
  protected:
    ...
    void verifyInvariants(const std::string& context) {
        ...
        EXPECT_LE(editor.cursor, editor.length)
            << context << ": cursor (" << editor.cursor << ") exceeds length ("
            << editor.length << ")";
 ...
 * **Validates: Requirements 4.1-4.15**
 */
TEST_F(LineEditorPropertyTest,
       Property4_BufferConsistencyAfterRandomOperations) {
    for (int iter = 0; iter < PROPERTY_TEST_ITERATIONS; ++iter) {
        ...
        for (int op = 0; op < opCount; ++op) {
            ...
            std::string opContext =
                "Iter " + std::to_string(iter) + ", Op " + std::to_string(op);
...
TEST_F(LineEditorPropertyTest, Property4a_InsertPreservesContent) {
    for (int iter = 0; iter < PROPERTY_TEST_ITERATIONS; ++iter) {
        ...
        std::uniform_int_distribution<int> posDist(
            0, static_cast<int>(initial.length()));
        ...
        if (inserted) {
            ...
            EXPECT_STREQ(expected.c_str(), editor.buffer)
                << "Iter " << iter << ": content mismatch after insert at pos "
                << insertPos;
...
/**
 * Feature: shell-cli-middleware, Property 4b: Backspace Removes Correct
 * Character
 *
...
TEST_F(LineEditorPropertyTest, Property4b_BackspaceRemovesCorrectChar) {
    for (int iter = 0; iter < PROPERTY_TEST_ITERATIONS; ++iter) {
        ...
        std::uniform_int_distribution<int> posDist(
            1, static_cast<int>(initial.length()));
        ...
        EXPECT_TRUE(deleted)
            << "Iter " << iter << ": backspace should succeed at pos "
            << cursorPos;
        EXPECT_STREQ(expected.c_str(), editor.buffer)
...
TEST_F(LineEditorPropertyTest, Property4c_DeleteRemovesCorrectChar) {
    for (int iter = 0; iter < PROPERTY_TEST_ITERATIONS; ++iter) {
        ...
        std::uniform_int_distribution<int> posDist(
            0, static_cast<int>(initial.length()) - 1);
        int cursorPos = posDist(rng);
        line_editor_move_to_start(&editor);
        line_editor_move_cursor(&editor, cursorPos);
        ...
        EXPECT_TRUE(deleted) << "Iter " << iter
                             << ": delete should succeed at pos " << cursorPos;
        EXPECT_STREQ(expected.c_str(), editor.buffer)
...
TEST_F(LineEditorPropertyTest, Property4d_CursorMovementClamping) {
    for (int iter = 0; iter < PROPERTY_TEST_ITERATIONS; ++iter) {
        ...
        EXPECT_GE(editor.cursor, 0)
            << "Iter " << iter << ": cursor went negative with offset "
            << offset;
        EXPECT_LE(editor.cursor, editor.length)
            << "Iter " << iter << ": cursor exceeded length with offset "
            << offset;
