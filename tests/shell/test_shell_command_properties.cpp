...
TEST_F(CommandPropertyTest, Property2_CommandRegistrationRoundTrip) {
    for (int iter = 0; iter < PROPERTY_TEST_ITERATIONS; ++iter) {
        ...
        shell_command_t cmd = {.name = s_name,
        ...
        EXPECT_EQ(nullptr, shell_get_command(s_name))
            << "Iter " << iter
            << ": command still retrievable after unregister";
...
TEST_F(CommandPropertyTest, Property2b_DuplicateRegistrationRejection) {
    for (int iter = 0; iter < PROPERTY_TEST_ITERATIONS; ++iter) {
        ...
        shell_command_t cmd1 = {.name = s_name,
                                .handler = test_handler,
                                .help = "First command",
                                .usage = nullptr,
                                .completion = nullptr};

        shell_command_t cmd2 = {.name = s_name,
                                .handler = test_handler,
                                .help = "Second command",
                                .usage = nullptr,
                                .completion = nullptr};
...
TEST_F(CommandPropertyTest, Property2c_UnregisterNonExistentFails) {
    for (int iter = 0; iter < PROPERTY_TEST_ITERATIONS; ++iter) {
        ...
        shell_command_t cmd = {.name = s_regName,
                               .handler = test_handler,
                               .help = nullptr,
                               .usage = nullptr,
                               .completion = nullptr};
...
TEST_F(CommandPropertyTest, Property2d_CapacityLimitEnforcement) {
    for (int iter = 0; iter < PROPERTY_TEST_ITERATIONS; ++iter) {
        ...
        std::vector<std::string> names =
            generateUniqueCommandNames(SHELL_MAX_COMMANDS + 1);
...
TEST_F(CommandPropertyTest, Property2e_ReregistrationAfterUnregister) {
    for (int iter = 0; iter < PROPERTY_TEST_ITERATIONS; ++iter) {
        ...
        for (int c = 0; c < cycles; ++c) {
            ...
            EXPECT_EQ(nullptr, shell_get_command(s_name))
                << "Iter " << iter << ", cycle " << c
                << ": command should not be retrievable";
        }
    }
}
