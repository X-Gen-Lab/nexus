 ...
 * \details         Requirements 8.2 - Lock on unlocked mutex should succeed immediately
 */
...
TEST_F(OsalMutexTest, LockWithNullHandle) {
    EXPECT_EQ(OSAL_ERROR_NULL_POINTER, osal_mutex_lock(nullptr, OSAL_WAIT_FOREVER));
}
 ...
 * \details         The mutex is configured as recursive, so same thread can lock multiple times
 */
...
TEST_F(OsalMutexTest, LockNoWaitWhenHeld) {
    ...
    EXPECT_EQ(OSAL_ERROR_TIMEOUT, osal_mutex_lock(s_shared_mutex, OSAL_NO_WAIT));
