...
TEST_F(OsalEventPropertyTest, Property2_SetBitsAtomicallyUpdatesState) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        EXPECT_EQ(bits_to_set, current_bits & bits_to_set)
            << "Iteration " << test_iter
            << ": all specified bits should be set "
            << "(expected=0x" << std::hex << bits_to_set << ", got=0x"
    ...
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        std::thread waiting_task([&]() {
            ...
            osal_status_t status =
                osal_event_wait(handle, bits_to_wait, &options, nullptr);
        ...
        EXPECT_TRUE(task_woke_up)
            << "Iteration " << test_iter
            << ": task should wake up when bits are set "
...
/**
 * Feature: osal-event-flags, Property 4: Clear Bits Atomically Updates Only
 * Specified Bits
 *
 ...
 */
TEST_F(OsalEventPropertyTest,
       Property4_ClearBitsAtomicallyUpdatesOnlySpecifiedBits) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        osal_event_handle_t handle = nullptr;
        ASSERT_EQ(OSAL_OK, osal_event_create(&handle));
        ...
        EXPECT_EQ(expected_bits, current_bits)
            << "Iteration " << test_iter
            << ": only specified bits should be cleared "
...
TEST_F(OsalEventPropertyTest, Property5_WaitAllModeRequiresAllBits) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        EXPECT_EQ(OSAL_OK, status)
            << "Iteration " << test_iter
            << ": wait should succeed when all bits are set "
            << "(bits=0x" << std::hex << bits_to_wait << std::dec << ")";
        ...
        if (bits_to_wait != 1) { /* Skip if only one bit */
            ...
            if (partial_bits != 0 && partial_bits != bits_to_wait) {
                ...
                EXPECT_EQ(OSAL_ERROR_TIMEOUT, status)
                    << "Iteration " << test_iter
                    << ": wait should timeout when not all bits are set "
                    << "(waiting=0x" << std::hex << bits_to_wait << ", set=0x"
                    << partial_bits << std::dec << ")";
            }
        }
...
TEST_F(OsalEventPropertyTest, Property6_WaitAnyModeRequiresAnyBit) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        EXPECT_EQ(OSAL_OK, status)
            << "Iteration " << test_iter
            << ": wait should succeed when any bit is set "
            << "(waiting=0x" << std::hex << bits_to_wait << ", set=0x"
            << single_bit << std::dec << ")";
...
TEST_F(OsalEventPropertyTest, Property7_AutoClearClearsMatchedBits) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        osal_status_t status =
            osal_event_wait(handle, bits_to_wait, &options, &bits_out);
        EXPECT_EQ(OSAL_OK, status)
        ...
        EXPECT_EQ(expected_bits, current_bits)
            ...
            << "(initial=0x" << std::hex << all_bits << ", matched=0x"
            << bits_out << ", expected=0x" << expected_bits << ", got=0x"
            << current_bits << std::dec << ")";
...
TEST_F(OsalEventPropertyTest, Property8_NonAutoClearPreservesBits) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        osal_status_t status =
            osal_event_wait(handle, bits_to_set, &options, nullptr);
        EXPECT_EQ(OSAL_OK, status)
            << "Iteration " << test_iter << ": wait should succeed";
 ...
 * satisfied within T milliseconds, the operation SHALL return
 * OSAL_ERROR_TIMEOUT.
 *
...
TEST_F(OsalEventPropertyTest, Property9_WaitTimeoutReturnsError) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        osal_event_wait_options_t options = {.mode = randomWaitMode(),
...
TEST_F(OsalEventPropertyTest, Property10_WaitImmediateReturnWhenSatisfied) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        EXPECT_LT(elapsed_ms, 100)
            << "Iteration " << test_iter
            << ": should return quickly (elapsed=" << elapsed_ms << "ms)";
...
/**
 * Feature: osal-event-flags, Property 11: Get Returns Current Value Without
 * Modification
 *
 ...
 */
TEST_F(OsalEventPropertyTest,
       Property11_GetReturnsCurrentValueWithoutModification) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        osal_event_handle_t handle = nullptr;
        ASSERT_EQ(OSAL_OK, osal_event_create(&handle));
 ...
 * bits are set together without intermediate states visible to other
 * operations.
 *
...
TEST_F(OsalEventPropertyTest, Property12_SetOperationAtomicity) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        EXPECT_EQ(bits1, final_bits & bits1)
            ...
            << "(bits1=0x" << std::hex << bits1 << ", final=0x" << final_bits
            << std::dec << ")";
        EXPECT_EQ(bits2, final_bits & bits2)
            << "Iteration " << test_iter << ": bits2 should be set "
            << "(bits2=0x" << std::hex << bits2 << ", final=0x" << final_bits
            << std::dec << ")";
 ...
 * bits are cleared together without intermediate states visible to other
 * operations.
 *
...
TEST_F(OsalEventPropertyTest, Property13_ClearOperationAtomicity) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        EXPECT_EQ(expected_bits, final_bits)
            ...
            << "(initial=0x" << std::hex << initial_bits << ", cleared1=0x"
            << bits1 << ", cleared2=0x" << bits2 << ", expected=0x"
            << expected_bits << ", final=0x" << final_bits << std::dec << ")";
...
TEST_F(OsalEventPropertyTest, Property14_WaitCheckAndClearAtomicity) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        EXPECT_EQ(0u, final_bits & bits_to_wait)
            ...
            << "(waited=0x" << std::hex << bits_to_wait << ", after_wait=0x"
            << final_bits << std::dec << ")";
 ...
 * setting those bits SHALL wake all waiting tasks whose conditions are
 * satisfied.
 *
...
TEST_F(OsalEventPropertyTest, Property15_BroadcastWakeAllWaitingTasks) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        EXPECT_EQ(num_waiters, tasks_woke_up)
            << "Iteration " << test_iter
            << ": all waiting tasks should wake up "
            << "(bits=0x" << std::hex << bits_to_wait << std::dec << ")";
...
TEST_F(OsalEventPropertyTest, Property16_NullPointerErrorHandling) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        EXPECT_EQ(OSAL_ERROR_NULL_POINTER, osal_event_set(nullptr, bits))
            << "Iteration " << test_iter
            << ": set with NULL should return error";
        ...
        EXPECT_EQ(OSAL_ERROR_NULL_POINTER,
                  osal_event_wait(nullptr, bits, &options, nullptr))
            << "Iteration " << test_iter
            << ": wait with NULL should return error";
        ...
        EXPECT_EQ(OSAL_ERROR_NULL_POINTER,
                  osal_event_set_from_isr(nullptr, bits))
            << "Iteration " << test_iter
            << ": set_from_isr with NULL should return error";
    }
}
...
TEST_F(OsalEventPropertyTest, Property17_InvalidParameterErrorHandling) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        EXPECT_EQ(OSAL_ERROR_INVALID_PARAM,
                  osal_event_wait(handle, 0, &options, nullptr))
            << "Iteration " << test_iter
            << ": wait with zero mask should return error";

        /* Test set_from_isr with zero mask */
        EXPECT_EQ(OSAL_ERROR_INVALID_PARAM, osal_event_set_from_isr(handle, 0))
            << "Iteration " << test_iter
            << ": set_from_isr with zero mask should return error";
