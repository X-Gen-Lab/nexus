...
TEST_F(OsalErrorHandlingPropertyTest, Property12_TimeoutConversionCorrectness) {
     ...
     * When using OSAL_WAIT_FOREVER, blocking operations should wait indefinitely.
     * We verify this by checking that a mutex lock with OSAL_WAIT_FOREVER
     * succeeds when the mutex is available.
     */
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        /* Test OSAL_WAIT_FOREVER - should succeed immediately on available mutex */
...
TEST_F(OsalErrorHandlingPropertyTest, Property12_NoWaitBehavior) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        /* Try to receive from empty queue with NO_WAIT - should fail immediately */
        int item = 0;
...
TEST_F(OsalErrorHandlingPropertyTest, Property12_PositiveTimeoutBehavior) {
    ...
    for (int test_iter = 0; test_iter < 10; ++test_iter) {
        ...
        /* Elapsed time should be approximately the timeout value (within 50% tolerance) */
        EXPECT_GE(elapsed_ms, timeout_ms * 0.5)
            << "Iteration " << test_iter 
            << ": elapsed time (" << elapsed_ms << "ms) should be >= " 
            << (timeout_ms * 0.5) << "ms";
...
/**
 * Feature: freertos-adapter, Property 13 Extension: Semaphore Null Pointer Handling
 * 
...
TEST_F(OsalErrorHandlingPropertyTest, Property14_InvalidParameterErrorHandling) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        EXPECT_EQ(OSAL_ERROR_INVALID_PARAM, status)
            << "Iteration " << test_iter 
            << ": osal_task_create with priority " << (int)invalid_priority 
...
TEST_F(OsalErrorHandlingPropertyTest, Property14_QueueInvalidParameters) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        EXPECT_EQ(OSAL_ERROR_INVALID_PARAM, status)
            << "Iteration " << test_iter 
            << ": osal_queue_create with item_size=0 should return INVALID_PARAM";
        ...
        EXPECT_EQ(OSAL_ERROR_INVALID_PARAM, status)
            << "Iteration " << test_iter 
            << ": osal_queue_create with item_count=0 should return INVALID_PARAM";
...
/**
 * Feature: freertos-adapter, Property 14 Extension: Semaphore Invalid Parameters
 * 
 * *For any* counting semaphore creation with initial > max_count or max_count = 0,
 * the operation SHALL return OSAL_ERROR_INVALID_PARAM.
 * 
 * **Validates: Requirements 10.2**
 */
TEST_F(OsalErrorHandlingPropertyTest, Property14_SemaphoreInvalidParameters) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        EXPECT_EQ(OSAL_ERROR_INVALID_PARAM, status)
            << "Iteration " << test_iter 
            << ": osal_sem_create with initial(" << initial 
            << ") > max_count(" << max_count << ") should return INVALID_PARAM";
        ...
        EXPECT_EQ(OSAL_ERROR_INVALID_PARAM, status)
            << "Iteration " << test_iter 
            << ": osal_sem_create_counting with initial > max_count should return INVALID_PARAM";
        
        /* Test osal_sem_create_counting with max_count = 0 */
        status = osal_sem_create_counting(0, 0, &sem);
        EXPECT_EQ(OSAL_ERROR_INVALID_PARAM, status)
            << "Iteration " << test_iter 
            << ": osal_sem_create_counting with max_count=0 should return INVALID_PARAM";
    }
}
 ...
 * 
 * *For any* valid parameter combination, the operation SHALL succeed with OSAL_OK.
 * This is the inverse property - ensuring valid inputs work correctly.
 * 
 * **Validates: Requirements 10.2**
 */
