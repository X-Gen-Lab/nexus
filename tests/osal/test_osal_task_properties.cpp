...
TEST_F(OsalTaskPropertyTest, Property2_TaskLifecycleConsistency) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        osal_task_config_t config = {.name = name.c_str(),
...
TEST_F(OsalTaskPropertyTest, Property3_PriorityMappingCorrectness) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        osal_task_config_t config = {.name = "prio_test",
        ...
        if (status == OSAL_OK) {
            ...
            EXPECT_TRUE(waitForTaskCompletion(1000))
                << "Iteration " << test_iter << ": Task with priority "
...
TEST_F(OsalTaskPropertyTest, Property3_PriorityBoundaryValues) {
    ...
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        for (uint8_t priority : boundary_priorities) {
            ...
            osal_task_config_t config = {.name = "boundary_test",
                                         .func = property_test_task_func,
                                         .arg = nullptr,
                                         .priority = priority,
                                         .stack_size = 4096};
            ...
            EXPECT_EQ(OSAL_OK, status)
                << "Iteration " << test_iter
                << ": Task creation should succeed with boundary priority="
                << (int)priority;
...
TEST_F(OsalTaskPropertyTest, Property3_InvalidPriorityRejection) {
    ...
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        osal_task_config_t config = {.name = "invalid_prio",
                                     .func = property_test_task_func,
                                     .arg = nullptr,
        ...
        EXPECT_EQ(OSAL_ERROR_INVALID_PARAM, status)
            << "Iteration " << test_iter
            << ": Task creation should fail with invalid priority="
...
TEST_F(OsalTaskPropertyTest, Property4_TaskNamePreservation) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        osal_task_config_t config = {.name = expected_name.c_str(),
                                     .func = lifecycle_task_func,
                                     .arg = nullptr,
        ...
        if (status == OSAL_OK) {
            ...
            EXPECT_NE(nullptr, actual_name) << "Iteration " << test_iter
...
TEST_F(OsalTaskPropertyTest, Property4_NullNameHandling) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        osal_task_config_t config = {.name = nullptr, /* NULL name */
                                     .func = lifecycle_task_func,
                                     .arg = nullptr,
...
/**
 * Feature: freertos-adapter, Property 2 Extension: Multiple Suspend/Resume
 * Cycles
 *
...
TEST_F(OsalTaskPropertyTest, Property2_MultipleSuspendResumeCycles) {
    ...
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        osal_task_config_t config = {.name = "cycle_test",
                                     .func = lifecycle_task_func,
                                     .arg = nullptr,
                                     .priority = randomPriority(),
                                     .stack_size = 4096};
