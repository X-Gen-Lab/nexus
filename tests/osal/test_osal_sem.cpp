 ...
 * \details         Requirements 9.2 - Take should succeed immediately when count > 0
 */
...
TEST_F(OsalSemTest, TakeWithNullHandle) {
    EXPECT_EQ(OSAL_ERROR_NULL_POINTER, osal_sem_take(nullptr, OSAL_WAIT_FOREVER));
}
...
static void sem_consumer_task(void* arg) {
    ...
    while (!s_sem_producer_done || osal_sem_take(s_shared_sem, OSAL_NO_WAIT) == OSAL_OK) {
