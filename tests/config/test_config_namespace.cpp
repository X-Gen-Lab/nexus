...
TEST_F(ConfigNamespaceTest, OpenNamespaceNullHandle) {
    EXPECT_EQ(CONFIG_ERROR_INVALID_PARAM,
              config_open_namespace("test_ns", NULL));
}
...
TEST_F(ConfigNamespaceTest, NamespaceIsolationStr) {
    ...
    /* Verify isolation */
    EXPECT_EQ(CONFIG_OK,
              config_ns_get_str(ns1, "name", buffer, sizeof(buffer)));
    EXPECT_STREQ("Alice", buffer);

    EXPECT_EQ(CONFIG_OK,
              config_ns_get_str(ns2, "name", buffer, sizeof(buffer)));
...
TEST_F(ConfigNamespaceTest, NsGetI32InvalidHandle) {
    int32_t value = 0;
    EXPECT_EQ(CONFIG_ERROR_INVALID_PARAM,
              config_ns_get_i32(NULL, "key", &value, 0));
}

TEST_F(ConfigNamespaceTest, NsSetStrInvalidHandle) {
    EXPECT_EQ(CONFIG_ERROR_INVALID_PARAM,
              config_ns_set_str(NULL, "key", "value"));
}

TEST_F(ConfigNamespaceTest, NsGetStrInvalidHandle) {
    char buffer[64];
    EXPECT_EQ(CONFIG_ERROR_INVALID_PARAM,
              config_ns_get_str(NULL, "key", buffer, sizeof(buffer)));
}
...
TEST_F(ConfigNamespaceTest, NsTypeMismatch) {
    ...
    EXPECT_EQ(CONFIG_ERROR_TYPE_MISMATCH,
              config_ns_get_str(ns, "typed_key", buffer, sizeof(buffer)));
