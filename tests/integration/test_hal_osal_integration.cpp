...
TEST_F(HalOsalIntegrationTest, MultiTaskGpioWithMutex) {
    ...
    /* Create tasks */
    osal_task_config_t task1_config = {
        .name = "gpio_task1",
    ...
    osal_task_config_t task2_config = {
        .name = "gpio_task2",
...
static void consumer_task_func(void* arg) {
    ...
    for (int i = 0; i < num_messages && s_queue_test_state.running; i++) {
        ...
        if (osal_queue_receive(s_queue_test_state.queue, &msg, 1000) == OSAL_OK) {
...
TEST_F(HalOsalIntegrationTest, SemaphoreSynchronization) {
    /* Create semaphores */
    ASSERT_EQ(OSAL_OK, osal_sem_create(1, 1, &s_sem_sync_state.sem1));  /* Start with 1 */
    ASSERT_EQ(OSAL_OK, osal_sem_create(0, 1, &s_sem_sync_state.sem2));  /* Start with 0 */
    ...
    /* Create tasks */
    osal_task_config_t task_a_config = {
        .name = "sem_task_a",
    ...
    osal_task_config_t task_b_config = {
        .name = "sem_task_b",
...
TEST_F(HalOsalIntegrationTest, TimerWithTaskSync) {
    ...
    /* Set timer callback */
    ASSERT_EQ(HAL_OK, hal_timer_set_callback(HAL_TIMER_0, timer_callback, nullptr));
    
    /* Create waiting task */
    osal_task_config_t wait_task_config = {
        .name = "timer_wait",
    ...
    osal_task_config_t sim_task_config = {
        .name = "timer_sim",
...
static void server_task_func(void* arg) {
    ...
    while (s_multi_queue_state.running) {
        request_msg_t req;
        if (osal_queue_receive(s_multi_queue_state.request_queue, &req, 100) == OSAL_OK) {
            /* Process request */
            response_msg_t resp = {
...
static void client_task_func(void* arg) {
    ...
    for (int i = 0; i < num_requests && s_multi_queue_state.running; i++) {
        ...
        response_msg_t resp;
        if (osal_queue_receive(s_multi_queue_state.response_queue, &resp, 1000) == OSAL_OK) {
...
TEST_F(HalOsalIntegrationTest, RequestResponseQueues) {
    ...
    /* Create tasks */
    osal_task_config_t server_config = {
        .name = "server",
    ...
    osal_task_config_t client_config = {
        .name = "client",
