...
TEST_F(FlashPropertyTest, Property6_WriteWithoutEraseFails) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        EXPECT_EQ(NX_ERR_INVALID_STATE,
                  flash->write(flash, addr, data.data(), len))
            << "Iteration " << test_iter
...
TEST_F(FlashPropertyTest, Property6_WriteAfterEraseSucceeds) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        /* Erase then write */
        ASSERT_EQ(NX_OK, flash->erase(flash, addr, len));
        EXPECT_EQ(NX_OK, flash->write(flash, addr, data.data(), len))
            << "Iteration " << test_iter
...
TEST_F(FlashPropertyTest, Property7_PersistenceRoundTrip) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        ASSERT_EQ(NX_OK, flash->erase(flash, addr, len));
        ASSERT_EQ(NX_OK, flash->write(flash, addr, write_data.data(), len));
        ...
        ASSERT_EQ(NX_OK, flash->read(flash, addr, read_data.data(), len));
...
TEST_F(FlashPropertyTest,
    ...
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        ASSERT_EQ(NX_OK, flash->read(flash, addr, read_before.data(), len));
        ...
        ASSERT_EQ(NX_OK, flash->read(flash, addr, read_after.data(), len));
