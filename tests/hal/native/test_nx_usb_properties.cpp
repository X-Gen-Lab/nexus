...
TEST_F(USBPropertyTest, Property10_ConnectionStateConsistency) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        /* Ensure USB starts in connected state for each iteration */
...
TEST_F(USBPropertyTest, Property10_SuspendResumeStateConsistency) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        /* Ensure USB starts in running (not suspended) state for each iteration
         */
        bool initialized = false;
        bool suspended = false;
        ASSERT_EQ(NX_OK, native_usb_get_state(0, &initialized, &suspended));
        if (suspended) {
...
TEST_F(USBPropertyTest, Property10_PowerStateConsistency) {
    for (int test_iter = 0; test_iter < PROPERTY_TEST_ITERATIONS; ++test_iter) {
        ...
        /* Ensure power starts in disabled state for each iteration */
        if (power->is_enabled(power)) {
