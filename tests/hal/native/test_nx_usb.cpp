...
/*---------------------------------------------------------------------------*/
/* Power Callback Helpers                                                    */
/*---------------------------------------------------------------------------*/

static bool g_usb_power_callback_called = false;
static bool g_usb_power_callback_enabled = false;

static void usb_power_callback(void* user_data, bool enabled) {
    (void)user_data;
    g_usb_power_callback_called = true;
    g_usb_power_callback_enabled = enabled;
}
...
TEST_F(USBTest, PowerCallback) {
    ...
    /* Set callback */
    g_usb_power_callback_called = false;
    EXPECT_EQ(NX_OK, power->set_callback(power, usb_power_callback, nullptr));

    /* Enable power - should trigger callback */
    EXPECT_EQ(NX_OK, power->enable(power));
    EXPECT_TRUE(g_usb_power_callback_called);
    EXPECT_TRUE(g_usb_power_callback_enabled);

    /* Reset flags */
    g_usb_power_callback_called = false;
    g_usb_power_callback_enabled = false;

    /* Disable power - should trigger callback */
    EXPECT_EQ(NX_OK, power->disable(power));
    EXPECT_TRUE(g_usb_power_callback_called);
    EXPECT_FALSE(g_usb_power_callback_enabled);
}
