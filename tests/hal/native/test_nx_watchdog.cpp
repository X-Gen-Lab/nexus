...
TEST_F(WatchdogTest, NullPointerChecks) {
    ...
    nx_watchdog_t* null_wdt = nullptr;

    /* Calling functions on NULL should be guarded */
    if (null_wdt != nullptr) {
        null_wdt->start(null_wdt);
        null_wdt->stop(null_wdt);
        null_wdt->feed(null_wdt);
        null_wdt->get_timeout(null_wdt);
        null_wdt->set_callback(null_wdt, nullptr, nullptr);
    }

    /* Test that our implementation handles NULL gracefully */
    EXPECT_EQ(NX_ERR_NULL_PTR, wdt->start(nullptr));
    EXPECT_EQ(NX_ERR_NULL_PTR, wdt->stop(nullptr));
    EXPECT_EQ(0U, wdt->get_timeout(nullptr));
    EXPECT_EQ(NX_ERR_NULL_PTR, wdt->set_callback(nullptr, nullptr, nullptr));
}
...
TEST_F(WatchdogTest, DoubleInit) {
    ...
    /* Init again should succeed (idempotent) */
    EXPECT_EQ(NX_OK, lifecycle->init(lifecycle));
}
