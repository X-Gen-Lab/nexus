...
TEST_F(HalGpioTest, InitInvalidParams) {
    ...
    EXPECT_EQ(HAL_ERROR_NULL_POINTER, hal_gpio_init(HAL_GPIO_PORT_A, 0, nullptr));
}
 ...
 * \details         Requirements 1.1, 1.3, 1.4 - operations on uninitialized pins should fail
 */
TEST_F(HalGpioTest, OperationsOnUninitializedPin) {
    hal_gpio_level_t level;

    /* Write on uninitialized pin should fail */
    EXPECT_EQ(HAL_ERROR_NOT_INIT, hal_gpio_write(HAL_GPIO_PORT_A, 0, HAL_GPIO_LEVEL_HIGH));
...
TEST_F(HalGpioTest, WriteOnInputPin) {
    ...
    EXPECT_EQ(HAL_ERROR_INVALID_STATE, hal_gpio_write(HAL_GPIO_PORT_A, 1, HAL_GPIO_LEVEL_HIGH));
...
TEST_F(HalGpioTest, ReadNullPointer) {
    ...
    EXPECT_EQ(HAL_ERROR_NULL_POINTER, hal_gpio_read(HAL_GPIO_PORT_A, 2, nullptr));
}
