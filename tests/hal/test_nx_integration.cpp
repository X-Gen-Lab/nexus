...
class GPIOTimerIntegrationTest : public ::testing::Test {
  protected:
    void SetUp() override {
        ...
        gpio = nx_gpio_native_get(0, 0);
        ASSERT_NE(nullptr, gpio);

        /* Get Timer PWM instance */
        timer_pwm = nx_timer_pwm_native_get(0);
...
TEST_F(SPIFlashIntegrationTest, FlashEraseWriteRead) {
    ...
    EXPECT_TRUE(nx_flash_native_is_erased(0, sector_addr, 4096));
...
class USBUARTIntegrationTest : public ::testing::Test {
  protected:
    ...
    void TearDown() override {
        ...
        if (usb != nullptr) {
            nx_usb_native_simulate_disconnect(0);
        }
...
TEST_F(USBUARTIntegrationTest, USBCDCDataTransfer) {
    ...
    EXPECT_EQ(NX_OK, nx_usb_native_inject_rx(0, (const uint8_t*)usb_data,
                                             usb_data_len));
...
TEST_F(USBUARTIntegrationTest, USBCDCBidirectionalTransfer) {
    ...
    EXPECT_EQ(NX_OK, nx_usb_native_inject_rx(0, (const uint8_t*)usb_to_uart,
                                             strlen(usb_to_uart)));
...
TEST_F(USBUARTIntegrationTest, USBCDCLargeDataTransfer) {
    ...
    EXPECT_EQ(NX_OK,
              nx_usb_native_inject_rx(0, large_data, sizeof(large_data)));
