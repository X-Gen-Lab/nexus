...
config_status_t
config_init(const config_manager_config_t* config) {
    ...
    config_status_t status = config_store_init(g_config.max_keys,
                                                g_config.max_key_len,
                                                g_config.max_value_size);
    if (status != CONFIG_OK) {
        g_last_error = status;
        return status;
    }
...
config_status_t
config_get_last_error(void) {
    return g_last_error;
}

const char*
config_error_to_str(config_status_t status) {
...
config_status_t
config_exists(const char* key, bool* exists) {
    if (!g_config_initialized) {
        g_last_error = CONFIG_ERROR_NOT_INIT;
        return CONFIG_ERROR_NOT_INIT;
    }
...
config_status_t
config_get_type(const char* key, config_type_t* type) {
    if (!g_config_initialized) {
        g_last_error = CONFIG_ERROR_NOT_INIT;
        return CONFIG_ERROR_NOT_INIT;
    }
...
config_delete(const char* key) {
    ...
    config_status_t status = config_store_delete(key, CONFIG_DEFAULT_NAMESPACE_ID);
    g_last_error = status;
    return status;
}

config_status_t
config_get_count(size_t* count) {
    if (!g_config_initialized) {
        g_last_error = CONFIG_ERROR_NOT_INIT;
        return CONFIG_ERROR_NOT_INIT;
    }
...
config_status_t
config_set_backend(const config_backend_t* backend) {
    CONFIG_UNUSED(backend);
    return CONFIG_ERROR_NOT_INIT; /* Will be implemented in Task 10 */
}
